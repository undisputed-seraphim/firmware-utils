...
static int rtk_fix_4k_chk(char *inputFile, char *outFile)
{
	...
	pHeader->len =
		DWORD_SWAP((size - sizeof(IMG_HEADER_T) - JFFS2_ENDMARK_LEN));

	//erase old chksum
	//printf("old_cksum = %8x\n !!",*((unsigned short *)&buf[sizeof(IMG_HEADER_T)+old_linx_len-sizeof(checksum)]));
	*((unsigned short *)&buf[sizeof(IMG_HEADER_T) + old_linx_len -
				 sizeof(checksum)]) = 0;

	//add new chksum
	checksum = WORD_SWAP(calculateChecksum(
		buf + sizeof(IMG_HEADER_T),
		(size - sizeof(IMG_HEADER_T) - JFFS2_ENDMARK_LEN)));
...
int main(int argc, char **argv)
{
	...
	if (argc == 4 && !strcmp(argv[1], "size_chk")) {
		...
		printf("Image decompress end addr    :0x%x\n",
		       ((unsigned int)DEFAULT_BASE_ADDR) +
	...
	if (argc == 3 && !strcmp(argv[1], "flash_size_chk")) {
		...
		total_size =
			ROOT_IMAGE_OFFSET - CODE_IMAGE_OFFSET -
			CERT_SIZE; // To reserve CERT_SIZE for 802.1x wlan client mode to store 802.1x certs
#else
	...
	if (is_vmlinuxhdr) {
		size = status.st_size + sizeof(padding_len) +
		       sizeof(lchecksum) + sizeof(start_addr);
	...
	if (is_vmlinuxhdr) {
		...
		*((unsigned int *)((char *)pHeader + 4)) = start_addr;
		lchecksum = DWORD_SWAP(calculate_long_checksum(
		...
		if (!strcmp("root", argv[1]))
			...
			memcpy(pHeader->signature, FW_HEADER_WITH_ROOT,
			       SIGNATURE_LEN);
...
#define SIZE_OF_SQFS_SUPER_BLOCK 640
			unsigned int fs_len;
			fs_len = DWORD_SWAP((size - sizeof(IMG_HEADER_T) -
					     sizeof(checksum) -
		...
		*((unsigned short *)&buf[size - sizeof(IMG_HEADER_T) -
					 sizeof(checksum)]) = checksum;
	} else { // is_vmlinux=1
		*((unsigned int *)pHeader) = DWORD_SWAP(padding_len);
		lchecksum = DWORD_SWAP(
			calculate_long_checksum((unsigned int *)buf, size - 8));
	...
	if (is_vmlinuxhdr)
		printf("Generate image successfully, length=%d, checksum=0x%x, padding=%d, start address=0x%08x\n",
		       size - 12 - padding_len, lchecksum, padding_len,
		       DWORD_SWAP(start_addr));
	else if (!is_vmlinux)
