...
#define BYTE_SWAP(word) \
	(((word >> 8) & 0xff) | ((((unsigned char)word) << 8) & 0xff00))
...
static TYPE_T checkInputFile(char *filename, int *pWith_header)
{
	...
	*pWith_header = 1;
	if (!memcmp(signature, BOOT_HEADER, 4))
		...
		return CONFIG;
	else if (!memcmp(signature, "6G", 2) || !memcmp(signature, "6A", 2) ||
		 !memcmp(signature, "6g", 2) || !memcmp(signature, "6a", 2) ||
		 !memcmp(signature, "COMPDS", 6)) {
		no_hw_config = 1;
		return CONFIG;
...
int main(int argc, char **argv)
{
...
#endif
		{
			...
			if (is_all)
				checksum = calculateChecksum(
					(char *)buf, sbuf.st_size, checksum);
			else {
				...
				if (!cascade && last_idx != -1 &&
				    sector[i].with_header) {
					if ((sector[last_idx].offset +
					     sector[last_idx].size) <
					    burnAddr) {
						len = burnAddr -
						      (sector[last_idx].offset +
						       sector[last_idx].size);
						buf1 = calloc(len, 1);
						if (buf1 == NULL) {
							printf("allocate buffer failed %d!\n",
	...
	if (is_all) {
		...
		checksum = calculateChecksum((char *)&allHeader,
					     sizeof(allHeader), checksum);
